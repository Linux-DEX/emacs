#+TITLE: Linux-DEX GNU Emacs Config
#+AUTHOR: xander
#+DESCRIPTION: xander personal Emacs config.
...
- [[#neotree][NEOTREE]]
- [[#org-mode][ORG MODE]]
  - [[#eanbling-table-of-contents][Eanbling table of Contents]]
...
- [[#changing-list-style][Changing list Style]]
...
* DASHBOARD
Emacs Dashboard is an extensible startup screen showing you recent files, bookmarks, agenda items and a Emacs banner.
...
** Magit
Magit is a fult-featured git client for Emacs.
#+begin_src emacs-lisp
(use-package magit)
#+end_src
...
* MODELINE
The modeline is the bottom status bar that appears in Emacs windows. While you can create you won custom modeline, why go to the trouble when Doom emacs already has a nice modeline package available. For more information on what is available to configure in the Doom modeline, check out: Doom Modeline.
#+begin_src emacs-lisp
...
* PERSPECTIVE
Perspective provides multiple named workspaces (or "perspective") in Emacs, similar to multiple desktops in windows managers. Each persective has its own buffer list and its own windows layout, along with some other isolated niceties, like the xref ring.
#+begin_src emacs-lisp
